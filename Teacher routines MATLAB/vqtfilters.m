function [g,a,fc,L,info]=vqtfilters(fs,fmin,fmax,bins,Ls,varargin)
...
fsupp(1) = 2*fmin;
fsupp(2) = (fc(2))*(2^(1/bins(1))-2^(-1/bins(1)))+kv.gam;

for k = [3:M , M]
    %fsupp(k) = (fc(k+1)-fc(k-1))/fmin*fc(k)+kv.gam;
    fsupp(k) = (fc(k+1)-fc(k-1))+kv.gam;
end

fsupp(M+1) = (fc(M+1))*(2^(1/bins(end))-2^(-1/bins(end)))+kv.gam;
